...
const template = (id) => `
	<button id="btn-${id}-dropselect"
		type="button"
		class="btn btn-success dropdown-toggle dropselect-open"
...
class DropSelect {
	...
	set_button_text(text) {
		this.container.find('.dropselect-open').text(text);
	}
	...
	select(option, trigger) {
		...
		elems = this.container.find('.dropselect-menu').children();
		for (let e of elems) {
		...
		if (trigger) { this.trigger('select', { option: option }); }
	}
	...
	trigger(name, data) {
		this.container.trigger(`component.dropselect.${name}`, data);
	}
}
