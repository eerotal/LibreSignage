...
class Slide {
	...
	*/
	...
	/**
	* Copy a Slide into a specific Queue.
	*
	* @param {string} dest_name The destination Queue name.
	*
	* @return {Slide} The duplicate Slide object.
	*/
	async copy(dest_name) {
		let resp = await this.api.call(
			APIEndpoints.SLIDE_COPY,
			{
				'id': this.get('id'),
				'dest': dest_name
			}
	...
	*/
	add_to_queue(name) {
		queues = this.get('queue_names');
		queues.push(name);
		this.set({'queue_names': queues});
	}

	/**
	* Remove a Slide from a Queue.
	*
	* This function succeeds even if the Slide doesn't exist in the
	* specified Queue.
	*
	* @param {string} name The name of the Queue.
	*/
	remove_from_queue(name) {
		queues = this.slide.get('queue_names');
		i = queues.indexOf(name);
		if (i !== -1) {
			queues.splice(i, 1);
			this.set({'queue_names': queues});
		}
