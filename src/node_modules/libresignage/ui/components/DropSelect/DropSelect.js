...
class DropSelect {
	...
	*
	* @param {HTMLElement} The container where the DropSelect is created.
	*/
	constructor(container) {
		Assert.assert(container.id != '', 'Container element must have an ID.');
		...
		this.container.addEventListener(('show.bs.dropdown'), e => {
			this.container.dispatchEvent('component.dropselect.show')
		});

		this.container.addEventListener(('hide.bs.dropdown'), e => {
			this.container.dispatchEvent('component.dropselect.hide')
		});
	}
	...
	*/
	set_options(options, def) {
		...
		for (let o of options) {
			...
			`;
			template.content.addEventListener('click', () => {
				this.select(o, true)
			});
			dropselect_menu.innerHTML += template.content.firstElementChild;
		}
	...
	*/
	select(option, dispatch_event) {
		Assert.assert(this.options != null, 'No options set.');
		Assert.assert(this.options.includes(option), 'No such option.');

		let elems = null;
	
		this.selected = option;
		elems = [...this.container.querySelector('.dropselect-menu').children];
		for (let e of elems) {
		...
		if (dispatch_event) {
			this.container.dispatchEvent('component.dropselect.select');
		}
	}
