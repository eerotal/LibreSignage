...
exports.User = class User {
	...
	*/
	...
	set(user, groups, pass, keys) {
		if (!user) {
			throw new Error("Invalid username for User object.");
		}
		this.user = user;
		this.groups = groups;
		this.pass = pass;
	...
	save(ready_callback) {
		...
		*/
		...
		this.api.call(this.api.ENDP.USER_SAVE, data, (resp) => {
			if (ready_callback) {
	...
	load(user, ready_callback) {
		...
		*/
		...
		if (user == null) {
			this.api.call(
				this.api.ENDP.USER_GET_CURRENT,
				null,
				cb
			);
		} else {
			this.api.call(
				this.api.ENDP.USER_GET,
				{'user': user},
				cb
			);
		}
	...
	remove(ready_callback) {
		...
		*/
		this.api.call(this.api.ENDP.USER_REMOVE, {'user': this.user},
			(resp) => {
				if (ready_callback) {
					ready_callback(resp.error);
				}
	...
	gen_key(ready_callback) {
		...
		*/
		this.api.call(this.api.ENDP.USER_GENERATE_KEY, null,
			(resp) => {
	...
	rm_key(rkey, ready_callback) {
		...
		*/
		this.api.call(this.api.ENDP.USER_REMOVE_KEY, {key: rkey},
			(resp) => {
				if (resp.error == API_E.API_E_OK) {
...
exports.user_get_by_name = function(user) {
	_usermgr_error_on_not_ready();
	var usrs = exports.users_get();
	for (var u in usrs) {
...
exports.users_add = function(user) {
	_usermgr_error_on_not_ready();
	_usermgr_users.push(user);
}

exports.users_load = function(api, ready_callback) {
	/*
	...
	*/
	api.call(api.ENDP.USERS_GET_ALL, null, (resp) => {
		...
		if (ready_callback) { ready_callback(_usermgr_users); }
	});
}

exports.user_exists = function(user) {
	/*
