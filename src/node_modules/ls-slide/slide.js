...
exports.Slide = class Slide {
	...
	load(id, lock, lock_renew, callback) {
		...
		*/
		...
		if (lock) {
			...
			this.fetch(err => { if (callback) { callback(err); } });
		}
	}
	...
	fetch(callback) {
		...
		*/
		assert.assert('id' in this.data && this.data.id !== null);
		this.api.call(
	...
	lock_acquire(renew, callback) {
		...
		*/
		...
		console.log(`Slide: Lock '${this.data.id}'.`);
		this.api.call(
			...
			resp => {
				if (!resp.error && renew) {
					let ren_time = resp.expire
						...
						- Date.now()/1000;
					console.log(
						`Slide: Lock renewal in ${ren_time} seconds.`
					);
