...
var SlideLock = require('./slidelock.js').SlideLock;
...
class Slide {
	...
	async lock_acquire(renew) {
		...
		*/
		...
		this.data.lock = new SlideLock(resp.lock.session, resp.lock.expire);
	...
	is_locked_from_here() {
		...
		*  This function also checks that the lock is not expired.
		*  Returns true if the slide is locked and false otherwise.
		*/
		let lock_session_data = null;
		let api_session = null;

		if (this.data != null && this.data.lock != null) {
			lock_session_data = this.data.lock.get_session_data();
			api_session = this.api.get_session();
			return (
				!this.data.lock.is_expired()
				&& (
					lock_session_data.id === api_session.get_id()
					|| lock_session_data.id === api_session.get_orphan_master()
				)
			);
		} else {
			return false;
	...
	get_asset_uri(name) {
		/*
		*  Get the URI for the asset 'name'.
		*/
		assert(this.data.id != null, "Unsaved slide.");
	...
	get_asset_thumb_uri(name) {
		/*
		*  Get the URI for the thumbnail of the asset 'name'.
		*/
		assert(this.data.id != null, "Unsaved slide.");
	...
	has_thumb(name) {
		/*
		*  Check whether the slide has a thumbnail for the
		*  asset 'name'.
		*/
		assert(this.data.id != null, "Unsaved slide.");
		for (let a of Object.values(this.data.assets)) {
			if (a.filename === name) {
				return a.has_thumb;
			}
		}
	...
	set(data)  {
		...
		// Manually create the SlideLock instance if the slide is locked.
		if (data.lock != null) {
			this.data.lock = new SlideLock(data.lock.session, data.lock.expire);
		}
	}
