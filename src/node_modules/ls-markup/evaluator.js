...
function get_argtype(arg) {
	/*
	*  Get the type of 'arg' as defined in defs.ARGTYPES.
	*/
	if (arg.match(/^[0-9]+\%$/)) {
		return defs.ARGTYPES.percent;
	} else if (arg.match(/^[0-9]*$/)) {
		return defs.ARGTYPES.int;
	} else if (
		arg.match(/^\'.*\'$/)
		|| arg.match(/^\".*\"$/)
	) {
		return defs.ARGTYPES.str;
	} else {
		return defs.ARGTYPES.rstr;
	}
}
...
function chk_args(node) {
	...
	*/
	let expr = defs.EXPRESSIONS[node.name];
	let atype = null;
	let typestr = '';
...
function eval_node(node) {
	...
	*/
	...
	if (!(node.name in defs.EXPRESSIONS)) {
		throw new err.MarkupSyntaxError(
			'ETAG',
			node.lexeme,
			node.name
	...
	expr = defs.EXPRESSIONS[node.name];
	chk_args(node);
