...
/*
*  Define window with JSDOM when used standalone.
*/
var JSDOM = null;
...
module.exports.EXPRESSIONS = {
	...
	__inner__: {
		...
		handle: (node) => {
			// Sanitize HTML tags etc.
			let tmp = window.document.createElement('div');
			tmp.appendChild(window.document.createTextNode(node.data.text));
	...
	bgcolor: {
		...
		handle: (node) => {
			...
			*/
			let opt = new Option();
			opt.style.color = node.data.color;
			if (opt.style.color.length === 0) {
				throw new err.MarkupSyntaxError(
					'EVAL',
					node.lexeme,
					`Invalid color '${node.data.color}'.`
				);
			}
			...
			return [
				`<style>
					body {
						background-color: ${node.data.color} !important;
					}
				</style>`,
				``
			];
