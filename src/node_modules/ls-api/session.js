		...
		*/
		...
		if (
			...
			try {
				...
				if (e.code === APIError.err('API_E_NOT_AUTHORIZED')) {
			...
			/*
			*  Add the session token to the data dict because the
	...
	schedule_renewal() {
		...
		if (max_age <= APISession.RENEWAL_HEADROOM + 10) {
			throw new APIError(
				...
				`Maximum session age too low! Should be *at least*` +
				`${APISession.RENEWAL_HEADROOM + 10} seconds.`
			);
		}
