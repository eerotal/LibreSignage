...
exports.API = class API {
	...
	call(endpoint, data, callback) {
		...
		*/
		...
		if (!['POST', 'GET'].includes(endpoint.method)) {
			throw new Error(
		...
		switch (endpoint.req_mime) {
			case endpoints.MIME.JSON:
				ajax_settings.data = JSON.stringify(data);
				break;
			case endpoints.MIME.X_WWW_FORM_URLENCODED:
				ajax_settings.data = data;
				break;
			case endpoints.MIME.FORM_DATA:
				ajax_settings.data = data;
			...
			default:
				throw new Error(
					`Invalid endpoint MIME type '${endpoint.req_mime}'.`
				);
		}
